...
_METHODS = ('cg', 'genetic', 'tnc', 'momentum', 'rprop', 'bfgs')
...
class NeuralNet(Container):
    ...
    n_hidden_nodes = Int(4, iotype='in',
                         desc='Number of nodes in hidden layer of network')

    method = Enum(_METHODS[0], values=_METHODS, iotype='in',
                  desc='Training method')
    ...
    def get_uncertain_value(self, value):
        """Returns the value iself. Neural network can provide its own
        uncertainty. """
        return value
